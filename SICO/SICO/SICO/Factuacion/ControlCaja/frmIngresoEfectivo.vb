...
Public Class frmIngresoEfectivo
    ...
    Private Sub frmIngresoEfectivo_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
    ...
    Private Sub PanelAccion1_Guardar() Handles PanelAccion1.Guardar
        Try
            ...
            f.ShowDialog()
            If f.Autorizo Then
                If MessageBox.Show("¿Esta seguro de ingresar este efectivo?", "Advertencia", MessageBoxButtons.YesNo) = Windows.Forms.DialogResult.Yes Then
                    ...
                    If txtEfectivo.Text <> String.Empty Then
                        If _controlCaja.AperturaCaja(PanelAccion1.Usuario.Id, dteFecha.Value, "") Then
                            ...
                            _controlCaja.RealizarIngresoEfectivo(Me.PanelAccion1.Usuario.Id, txtEfectivo.Text, dteFecha.Value, PanelAccion1.sucursal.Id, Me.PanelAccion1.Usuario)
                            MessageBox.Show("Se ingreso correctamente el efectivo", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information)
                            Me.Close()
                        End If
                ...
                End If
            Else
                Throw New ApplicationException("No ser realizo el ingreso de efectivo")
            End If
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
